...
> **Current visual-upgrade mission note:** this remains the authoritative testing/deployment/network-safety runbook. For ordinary model/material/prop replacement that does not alter networking contracts, prefer local parse/relevant scene checks while iterating, then use the full local gate at coherent milestones. Do not turn visual work into networking/deployment work unless the visual change actually touches those contracts.
...
- Internal game server listener: `127.0.0.1:24567`
- Internal account API listener: `127.0.0.1:8080`
...
- Remote account releases: `/opt/creepy-pasta-account/releases`
- Account database: `/var/lib/creepy-pasta/accounts.db`
...
Online clients must authenticate with Google and obtain a short-lived, one-use game ticket before the Godot server accepts lobby or gameplay RPCs. Account/API, ticket, or authenticated session changes therefore require deploying the backend, dedicated server, Web client, and Caddy configuration as one matched release. See `docs/account_auth_oracle.md`.
...
8. Verify the public HTTPS/WSS edge and the account API:
...
Oracle Cloud Console must allow inbound TCP `80` and `443` (plus administrative SSH `22`). Remove the old public `24567` ingress rule: both Godot `24567` and account API `8080` are loopback-only and are reached through Caddy on `443`.

## Do Not Claim Fixed Until

- The local project starts headless without script errors.
- Backend unit tests pass and the account API is ready.
- The Oracle service is `active (running)` after redeploy when multiplayer/server code changed.
- Caddy is `active (running)`, `https://creepy-pasta.duckdns.org` loads, and the public WSS handshake succeeds.
- `24567` and `8080` listen only on Oracle loopback.
