...
- `scenes/next_place.tscn` follows the canonical `Environment/Architecture/{Floors,Walls,Ceilings,Openings}` hierarchy while retaining direct `PressurePlate`, `DialogueNpcs`, `Notes`, and `LevelExit` contracts.
