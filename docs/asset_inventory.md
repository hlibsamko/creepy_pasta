...
| --- | --- | --- | --- | --- |
| Poly Haven Wooden Bookshelf Worn, Ulan Cabanilla | `scenes/props/house_worn_bookshelf.tscn` | `scenes/endless_house/house_survey.tscn`: `Environment/Props/HouseWornBookshelf`, position `(8, 0, 30.9)`, against the south corridor wall | Blender 5.1.1 retained approximately 10k source triangles, normalized height to 2.1m and capped textures at 512px. Recipe: `tools/asset_pipeline/recipes/house_bookshelf.json`. Source: https://polyhaven.com/a/wooden_bookshelf_worn | Authored placement; Godot import, visual, collision and performance QA not run. Blender emitted a texture sampler warning that requires visual review |
| FurniMesh Transitional Wood Brass Floor Lamp | `scenes/props/house_wood_brass_floor_lamp.tscn` | `scenes/endless_house/house_survey.tscn`: `Environment/Props/HouseFloorLamp`, position `(18.1, 0, 27.1)`, beside the generated sideboard | Blender 5.1.1 normalized to 1.74m with ground pivot, requested geometry reduction to 18k-triangle budget and capped textures at 512px. Recipe: `tools/asset_pipeline/recipes/house_floor_lamp.json`. Source: https://furnimesh.com/library/lighting/floor-lamp/a-floor-lamp-featuring-a-dark-stained-w5picd/ | Authored placement; Godot import, visual, collision and performance QA not run. Blender emitted a texture sampler warning that requires visual review |
| Poly Haven Vintage Suitcase, Maximilian Schuster | `scenes/props/hotel_vintage_suitcase.tscn` | `scenes/branches/endless_hotel/endless_hotel_hall.tscn`: `Environment/Props/HotelVintageSuitcase`, position `(10, 0, 24)`, yaw `25°`, beside NightRadio | Blender 5.1.1 extracted one suitcase variant, named body/clasps/handle/lid, width 0.8m and texture cap 512px. Recipe: `tools/asset_pipeline/recipes/hotel_suitcase.json`. Source: https://polyhaven.com/a/vintage_suitcase | Authored placement; Godot import, visual, collision and performance QA not run. Blender emitted a texture sampler warning that requires visual review |
| 3D Assets open-top office waste bin | `scenes/props/mall_waste_bin.tscn` | `scenes/branches/empty_mall/empty_mall_concourse.tscn`: `Environment/Props/MallWasteBin`, position `(30.2, 0, 24)`, yaw `180°`, beside vending cabinet | Blender 5.1.1, height 0.88m, centered ground pivot; cosmetic strip shares liner metal, dark body/cavity preserved. Recipe: `tools/asset_pipeline/recipes/mall_waste_bin.json`. Source: https://3dassets.dev/assets/office-lobby-and-facilities-office-waste-bin-4f708eb2 | Authored placement only; Godot import, visual, collision and performance QA not run. Separate approximate box collision; no FPS/draw-call improvement claimed |
| Valentin Laffitte Retro Vending Machine | `scenes/props/mall_vending_machine.tscn` | `scenes/branches/empty_mall/empty_mall_concourse.tscn`: `Environment/Props/MallVendingMachine`, position `(28, 0, 24)`, beside WaitingChair | Blender 5.1.1, height 2.1m, centered ground pivot; wrapper facing -Z. Cosmetic solids remapped, five textured surfaces retained; explicit eight-material-identity exception to initial target three. Recipe: `tools/asset_pipeline/recipes/mall_vending_machine.json`. Source: https://valentin-laffitte.itch.io/retro-vending-machine-3d-asset | Authored placement; Godot import, visual, collision and performance QA not run. No FPS or draw-call improvement claimed |
...
Hotel suitcase temporary cleanup removed its 1K glTF and seven declared dependency files from `.asset_work/hotel_suitcase/`, freeing 4,294,734 bytes. The final GLB, source URL and recipe remain. Recovery requires another download and the same recipe; future availability is not guaranteed.

House floor-lamp temporary cleanup removed only its owned source GLB from `.asset_work/house_floor_lamp/`, freeing 15,586,260 bytes. The final GLB, source URL and recipe remain; recovery requires re-downloading and rerunning the recipe, subject to future availability.

House bookshelf temporary cleanup removed its 1K glTF and four declared dependencies from `.asset_work/house_bookshelf/`, freeing 2,777,194 bytes. The final GLB, source URL and recipe remain; recovery requires re-downloading and rerunning the recipe, subject to future availability.
...
Waste-bin temporary cleanup removed only `.asset_work/mall_waste_bin/waste_bin_source.glb`, freeing 29,176 bytes. Final production GLB, source CDN URL and recipe are retained; recovery requires re-downloading and rerunning the recipe, subject to future external availability. No reduction of the published game was measured.

Vending-machine temporary cleanup removed only the owned downloaded RAR and extracted source GLB, freeing 1,335,102 bytes. The final model, source URL and recipe remain. Recovery requires re-downloading the author upload and rerunning the recipe; future external availability is not guaranteed. This is disk cleanup, not a measured reduction of the published game.
